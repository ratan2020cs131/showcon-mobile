...
const DropDown = ({ title, list }) => {
    ...
    return (
        <View style={{ width: '100%', position: 'relative', justifyContent: 'center' }}>
            <TouchableOpacity activeOpacity={0.5} style={{ width: '100%', height: 45, flexDirection: 'row', backgroundColor: '#E0E0E0', borderTopLeftRadius: 7, borderTopRightRadius: 7, paddingHorizontal: 10, borderBottomLeftRadius: open ? 0 : 7, borderBottomRightRadius: open ? 0 : 7, alignItems: 'center' }} onPress={toggleOpen}>
                <MaterialCommunityIcons name="movie-edit-outline" size={20} color="black" />
                {open ?
                    <TextInput placeholder={title} style={[GlobalStyles.input, GlobalStyles.normalText, { color: 'black', borderWidth: 0, paddingHorizontal: 8, flex: 1 }]} onChangeText={(value) => handleSearch(value)} /> :
                    <Text style={[GlobalStyles.normalText, { color: '#808080', fontSize: 17, borderWidth: 0, paddingHorizontal: 8, flex: 1 }]}>{title}</Text>
                }
                <FontAwesome name={open ? "caret-up" : "caret-down"} size={24} color="black" style={{ position: 'absolute', right: 10 }} />
            </TouchableOpacity>
            {
                open &&
